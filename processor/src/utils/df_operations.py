...
def parse_option_symbol(df: pd.DataFrame, symbol_col: str = SYMBOL_COL) -> pd.DataFrame:
    ...
    return df.assign(
        ...
        exp_date=lambda _: pd.to_datetime(split_series[1], format="%d%b%y").dt.strftime(
            "%Y-%m-%d"
        ),
...
def create_base_tables(
    df_base_daily: pd.DataFrame,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Creates base tables for stocks, options, and forex."""
    df_base = df_base_daily.assign(
        prior_price=lambda x: pd.to_numeric(x[PRIOR_PRICE_COL], errors="coerce").fillna(
            0.0
        ),
        current_price=lambda x: pd.to_numeric(
            x[CURRENT_PRICE_COL], errors="coerce"
        ).fillna(0.0),
