...
class IBStatementProcessor:
    ...
    def _process_mtm_summary(self) -> None:
        ...
        if df_forex is not None:
            ...
            df_forex = df_forex.rename(
                columns={"symbol": "currency", "prior_price": "exchange_rate_eur"}
            )[
                [
                    "asset_category",
                    "currency",
        ...
        self.processed_data.update(
            {"stocks": df_stocks, "options": df_options, "forex": df_forex}
        )
    ...
    def _calculate_metrics(self) -> None:
        ...
        option_value = (
            df_options["current_quantity"] * df_options["current_price"]
        ...
        option_credit = (
            (
                df_options[df_options["current_quantity"] < 0]["current_quantity"]
                * df_options[df_options["current_quantity"] < 0]["current_price"]
            ).sum()
            * OPTIONS_CONTRACT_MULTIPLIER
        ).round(2)
